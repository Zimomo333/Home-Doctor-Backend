...
public class Porder  implements Serializable {
	...
	public void setFreedTime(String freedTime) {
		SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		if (freedTime.contains("-")) {
			try {
				Date parse = simpleDateFormat.parse(freedTime);
				this.freedTime = String.valueOf(parse.getTime());
			} catch (Exception e) {
				this.freedTime = String.valueOf(new Date().getTime());
			}
		}
		else {
			try {
				this.freedTime = simpleDateFormat.format(new Date(Long.parseLong(freedTime)));
			}catch (Exception e) {
				this.freedTime =simpleDateFormat.format(new Date());
			}
		}
	}
